 ...
 * 
 */
public class TransactionDataSource extends MPOSDatabase {
	...
	public int countOrderStatusNotSuccess(String saleDate){
		...
		Cursor cursor = getReadableDatabase().rawQuery(
				...
				new String[]{
					String.valueOf(TransactionDataSource.TRANS_STATUS_NEW),
					String.valueOf(TransactionDataSource.TRANS_STATUS_HOLD),
					saleDate
				});
	...
	public OrderDetail getSummaryOrder(int sessId, String dateFrom, String dateTo) {
		...
		String[] whereArgs = new String[] { 
				...
				dateTo,
                String.valueOf(TransactionDataSource.TRANS_STATUS_SUCCESS),
				String.valueOf(ProductsDataSource.NORMAL_TYPE),
				String.valueOf(ProductsDataSource.SET_CAN_SELECT),
				String.valueOf(ProductsDataSource.CHILD_OF_SET_HAVE_PRICE),
				String.valueOf(ProductsDataSource.COMMENT_HAVE_PRICE)
			};
		if(sessId != 0){
	...
	private OrderDetail getSummaryVat(int transactionId) {
		...
		Cursor cursor = getReadableDatabase().rawQuery(
				...
				new String[] {
					String.valueOf(transactionId), 
					String.valueOf(ProductsDataSource.NO_VAT)
				});
		if (cursor.moveToFirst()) {
	...
	public String formatReceiptNo(int year, int month, int day, int id) {
		ComputerDataSource computer = new ComputerDataSource(getContext());
	...
	public void closeTransaction(int transactionId, int staffId, double totalSalePrice) {
		...
		if(feature != null){
			if(feature.getFeatureValue() == 1 && totalSalePrice == 0){
				...
				ShopDataSource shopDataSource = new ShopDataSource(mContext);
				double transVat = Utils.calculateVatAmount(totalSalePrice, shopDataSource.getCompanyVatRate(),
						shopDataSource.getCompanyVatType());
	...
	public List<OrderComment> listOrderComment(int transId, int ordId){
		...
		Cursor cursor = queryOrderDetail(
				...
				new String[]{
					...
					String.valueOf(ordId),
					String.valueOf(ProductsDataSource.COMMENT_HAVE_PRICE),
					String.valueOf(ProductsDataSource.COMMENT_NOT_HAVE_PRICE)
				});
		if(cursor.moveToFirst()){
			do{
